...
// 3D Scene Component
function StatueScene({ currentIndex }: { currentIndex: number }) {
  const statues = [
    {
      name: "Majestic Horse",
      price: "$3,499",
...
export function StatueSlider3D() {
  ...
  const statues = [
    { name: "Majestic Horse", price: "$3,499", description: "Bronze stallion in dynamic pose" },
    { name: "Regal Lion", price: "$4,299", description: "Powerful lion with detailed mane" },
    { name: "Elegant Woman", price: "$3,899", description: "Classical feminine grace in marble" },
  ]
  ...
  const prevSlide = () => {
    setCurrentIndex((prev) => (prev - 1 + statues.length) % statues.length)
  }

  return (
    <div className="relative w-full h-[700px] bg-gradient-to-br from-muted/20 to-background rounded-2xl overflow-hidden">
      <div className="absolute top-8 left-1/2 transform -translate-x-1/2 text-center z-20 max-w-4xl">
        ...
        <p className="text-base md:text-lg text-muted-foreground mb-6 max-w-2xl mx-auto leading-relaxed">
          Explore our curated collection of premium statues from world-renowned artists. Experience art in 3D.
        </p>
      </div>
      ...
      <Canvas camera={{ position: [0, 2, 8], fov: 50 }}>
        <Suspense fallback={null}>
          <StatueScene currentIndex={currentIndex} />
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex items-center gap-4">
        <Button variant="outline" size="icon" onClick={prevSlide} className="bg-background/80 backdrop-blur-sm">
        ...
        <Button variant="outline" size="icon" onClick={nextSlide} className="bg-background/80 backdrop-blur-sm">
      ...
      <div className="absolute bottom-20 left-6 bg-background/90 backdrop-blur-sm rounded-lg p-4 max-w-xs">
        <h3 className="font-heading text-xl font-bold mb-2">{statues[currentIndex].name}</h3>
        <p className="text-muted-foreground text-sm mb-3">{statues[currentIndex].description}</p>
        <div className="flex items-center justify-between">
          <span className="text-2xl font-bold text-primary">{statues[currentIndex].price}</span>
